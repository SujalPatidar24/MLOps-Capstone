...
MLFLOW_TRACKING_URI = "https://dagshub.com/SujalPatidar24/MLOps-Capstone.mlflow"
dagshub.init(repo_owner="SujalPatidar24", repo_name="MLOps-Capstone", mlflow=True)
mlflow.set_tracking_uri(MLFLOW_TRACKING_URI)
mlflow.set_experiment("Ridge class Hyperparameter Tuning")
...
def train_and_log_model(X_train, X_test, y_train, y_test, vectorizer):
    ...
    'solver': ['auto', 'sag', 'lsqr'], # Different solvers
    'tol': [1e-3, 1e-4, 1e-5]                 # Convergence tolerance
}
    
    with mlflow.start_run():
        grid_search = GridSearchCV(RidgeClassifier(), param_grid, cv=5, scoring="f1", n_jobs=-1)
        ...
        for params, mean_score, std_score in zip(grid_search.cv_results_["params"], 
            ...
            with mlflow.start_run(run_name=f"RC with params: {params}", nested=True):
...
# Best Params: {'alpha': 0.01, 'solver': 'sag', 'tol': 0.001} | Best F1 Score: 0.7913